...
4. (可跳过) 打开设置 (默认快捷键 `ctrl+,`)，打开自动保存，这样就不用担心忘记保存了
...
我们之后只讨论 aex+ (使用插件系统) 的自机编写。如果你仍停留在不支持插件系统的 ex+ 版本，那么可以略过关于 "入口文件" 的内容，以你所在版本的自带自机导入方法为准。
...
- 其他的根目录可以通过全局搜索找到 (vscode 打开 luastg 所在文件夹后，按 `ctrl+shift+f` 快捷键，搜索 `AddSearchPath`)
