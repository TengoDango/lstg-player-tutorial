...
- `tab.x` 是读取或修改表中元素的一个语法糖，对应的原始写法是 `tab["x"]`
- `function tab:init(...) ... end` 是定义表中函数的一个语法糖，相当于在函数的参数列表开头添加了一个 `self`，对应的原始写法是 `function tab.init(self, ...) ... end` {#method}
- `tab:init(...)` 是调用表中函数的一个语法糖，相当于在函数的参数列表开头添加了，对应的原始写法是 `tab.init(tab, ...)`
...
Lua 提供了一个语法糖来模拟面向对象语言中的写法，我们可以看下面的例子：
...
如果你已经理解了[上文的语法糖](#method)，你应该能够理解这个例子是如何工作的。
...
在创建对象时，对象不会获取类的方法，虽然可以通过 `self.class.dist()` 调用类方法，但 `self:dist()` 这样的语法糖是不适用的。
