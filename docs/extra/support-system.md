...
我们考虑三个方面的平滑过渡：高速/低速切换、火力等级改变、惯性跟随自机。
...
到现在为止，子机跟随自机的方式仍然是比较生硬的直接跟随。虽然好像这样也不是不可以，但我们还是想实现子惯性跟随自机的效果。
...
OK 这一期手写子机系统的内容大概就这些，完整代码见附录。默认子机系统的主要难点在于各种过渡的写法，初看大概很难完全理解，不过没关系，只要把握住线性插值的方法，在其他的自机需求也能发挥一定的作用。
