...
slist: {
  火力等级: integer --> 子机信息: {
    子机编号: integer --> 位置信息: nil or {
      高速-x, 高速-y, 低速-x, 低速-y
    }
  }
}
```
想不出怎么展示数据结构比较好，暂且就这样吧但愿能看懂

### `player.sp: table`

【自动更新，只读】子机实际位置表，根据 `slist` 进行更新。
```
sp: {
  火力等级: integer --> 各个子机: {
    子机编号: integer --> 子机信息: nil | {
      x, y, 有效性: 0 ~ 1
    }
  }
}
...
### `frame.updateSupport` 事件

根据 `slist` 属性计算 `sp` 属性。

由于风神录系统废弃，我们不用在意 `support == 5` 的分支。

如果自机被时停，或者 `slist` 属性不存在，那么以下逻辑不会执行。

- 设 `p` 为火力等级 (`int(player.support)`，对应 data 代码里的 `s - 1`)
- 设 `t` 为 `p` 的小数部分。`t` 用于平滑过渡
- 对每个子机 (编号为 `i`)，
  - 如果 `slist` 火力等级 `p` 子机 `i` 的位置信息不存在，而火力等级 `p + 1` 子机 `i` 的信息位置存在，
    - 根据 [`lh`](#player-lh) 属性将火力等级 `p + 1` 的高速位置和低速位置进行插值，得到位置 `{x,y}` 赋值给 `player.sp[i]`
    - 将 `player.sp[i][3]` 设为 `t`，表示该子机的有效性
    - 由于子机用的是 `p + 1` 火力等级的位置，`t` 越接近 1，实际火力越接近 `p + 1` 火力等级，子机的有效性就越强
  - 如果火力等级 `p`, `p + 1` 的位置信息都存在，
    - 根据 `lh` 属性和 `t` 对火力等级 `p`, `p + 1` 的高速和低速位置进行插值，得到位置 `{x,y}` 赋值给 `player.sp[i]`
    - 将 `player.sp[i][3]` 设为 1，表示该子机必然有效
- 总之要求火力等级 `p + 1` 的位置信息必须存在，所以我们需要设置火力等级为 0,1,2,3,4,5 的位置信息 (尽管火力等级不会到达 5)，即 `slist` 表的长度为 6。
