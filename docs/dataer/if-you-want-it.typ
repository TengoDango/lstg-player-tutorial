...
于是自机相关的文件如下:
+ #cross-ref("/docs/appendix/player-lua.typ")[`THlib\player\player.lua`] \
  浅层的自机逻辑, 涉及自机的基类以及一些自机相关的class和函数
+ #cross-ref("/docs/appendix/player-system-lua.typ")[`THlib\player\player_system.lua`] \
  深层的自机逻辑, 涉及大量的自机特有的属性和行为
+ `plugins\PlayerExtensions` \
  自带的灵梦, 魔理沙, 咲夜自机
+ #cross-ref("/docs/appendix/wisys-lua.typ")[`THlib\WalkImageSystem.lua`] \
